...
recovery_count="$(sql "with picked as (select id from staging_plans order by created_at desc limit 3), updated as (update staging_plans s set status='PROCESSING', claimed_at=now() - interval '1 hour' from picked p where s.id=p.id returning 1) select count(*) from updated;")"
